...
import { 
  ArrowLeft, Calendar, Clock, CreditCard, DollarSign, 
  Download, FileText, Image as ImageIcon, MapPin, 
  MoreVertical, Paperclip, Phone, Plus, TrendingDown, 
  TrendingUp, User, Wallet, CheckCircle2,
  Send, MessageCircle, Check, ChevronDown,
...
// Real DB Types
type BookingService = { id: number; service_name: string; price: number; quantity: number }
type BookingDetail = {
  id: number
...
export default function BookingDetailsPage() {
  ...
  const [availableServices, setAvailableServices] = useState<{id: number, service_name: string, price: number}[]>([])
  ...
  const [customExpenses, setCustomExpenses] = useState<{id: string, name: string}[]>([])
  ...
  useEffect(() => {
    setCustomExpenses(expenseCategories.map(c => ({ id: String(c.id), name: c.category_name })))
  }, [expenseCategories])
  ...
  const [deletingCustomExpenseId, setDeletingCustomExpenseId] = useState<string | null>(null)
  ...
  const handleUpdateCustomExpense = async () => {
    ...
    try {
      const res = await fetch(`/api/bookings/${bookingId}/expense-categories`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ categoryId: editingCustomExpenseId, category_name: editingCustomExpenseName.trim() }),
      })
      const data = await res.json()
      if (data.category) {
        setExpenseCategories(prev => prev.map(c => c.id === Number(editingCustomExpenseId) ? data.category : c))
  ...
  const handleDeleteCustomExpense = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/bookings/${bookingId}/expense-categories?categoryId=${id}`, { method: "DELETE" })
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/bookings/${bookingId}/expense-categories`).then(r => r.json()),
      fetch(`/api/services`).then(r => r.json()), // Global services for add-new
    ])
      .then(([detailRes, paymentsRes, expensesRes, categoriesRes, servicesRes]) => {
  ...
  const handleAddAdditionalCharge = async () => {
    ...
    try {
      ...
      if (data.charge && booking) {
        setBooking({ ...booking, additional_charges: [...booking.additional_charges, data.charge] })
        setNewChargeName("")
        setNewChargeQty("1")
        setNewChargeRate("")
      }
  ...
  const handleUpdateAdditionalCharge = async () => {
    if (!editChargeName.trim() || !editChargeRate || editingChargeId === null) return
    ...
    try {
      ...
      if (data.charge && booking) {
        setBooking({
          ...booking,
          additional_charges: booking.additional_charges.map(c => c.id === editingChargeId ? data.charge : c)
        })
  ...
  const handleDeleteAdditionalCharge = async (chargeId: number) => {
    ...
    try {
      const res = await fetch(`/api/bookings/${bookingId}/additional-charges?chargeId=${chargeId}`, { method: "DELETE" })
      if (res.ok && booking) {
        setBooking({
          ...booking,
          additional_charges: booking.additional_charges.filter(c => c.id !== chargeId)
        })
      }
    } finally {
  ...
  const handleSendUpdate = (method: "whatsapp" | "email") => {
    ...
    if (method === "whatsapp") {
      ...
      window.open(`mailto:${booking.customer.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`, "_blank")
    }
  ...
  if (loading) {
    return (
      <>
        ...
        <AppLoader label="Loading booking details..." className="min-h-[52vh] rounded-[2rem] bg-white/45" />
      </>
    )
  }
  ...
  const servicesTotal = booking.services.reduce((acc, s) => acc + (Number(s.price) * (s.quantity ?? 1)), 0)
  const additionalTotal = showAdditionalCharges 
    ? booking.additional_charges.reduce((acc, c) => acc + (Number(c.rate) * Number(c.quantity)), 0)
    : 0
  ...
  const profitMargin = grandTotal > 0 ? ((netProfit / grandTotal) * 100).toFixed(1) : "0.0"
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900">
              Booking #{booking.user_booking_index ?? booking.id}
            </h1>
            <p className="text-sm text-slate-500 flex items-center gap-2">
              <Calendar className="size-3" /> {new Date(booking.booking_date).toLocaleDateString()} 
              <span className="text-slate-300">|</span> 
              <Clock className="size-3" /> {booking.start_time.substring(0, 5)} - {booking.end_time.substring(0, 5)}
            </p>
          </div>
        </div>
        
        <div className="flex items-center gap-2">
          <Badge className={`rounded-lg px-3 py-1 font-semibold uppercase tracking-wider text-[10px] ${statusColors[booking.status]}`}>
            {booking.status}
          </Badge>
          <Button 
            onClick={() => setUpdateOpen(true)}
            className="rounded-xl h-9 bg-[#7c3aed] text-white hover:bg-[#6d28d9] shadow-md shadow-purple-950/20"
          >
            Send Update
          </Button>
        </div>
      ...
      {/* Customer Info Card */}
      <Card className="rounded-[1.75rem] border-slate-100 bg-white/80 shadow-md shadow-purple-950/5 mb-6 overflow-hidden">
        <CardContent className="p-0">
          <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100">
            <div className="p-6 flex items-center gap-4">
              <Avatar className="size-14 border-2 border-purple-100">
                <AvatarFallback className="bg-purple-50 text-purple-700 font-bold text-lg">
                  {booking.customer.customer_name.substring(0, 2).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div>
                <p className="text-sm text-slate-500 font-medium">Client</p>
                <p className="font-bold text-slate-900">{booking.customer.customer_name}</p>
                <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                  <span className="flex items-center gap-1"><Phone className="size-3" /> {booking.customer.phone}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 flex flex-col justify-center bg-purple-50/50">
              ...
              <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                ...
                ₹{totalPaid.toLocaleString()} paid so far
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Tabs defaultValue="quotation" className="w-full">
        <div className="w-full overflow-x-auto pb-2 hide-scrollbar">
          <TabsList className="flex sm:grid sm:grid-cols-4 w-max min-w-full sm:w-full max-w-md h-12 items-center justify-center rounded-2xl bg-slate-100/80 p-1 mb-4 sm:mb-6 text-slate-500 mx-auto sm:mx-0">
            <TabsTrigger value="quotation" className="flex-1 px-4 sm:px-0 rounded-xl data-[state=active]:bg-white data-[state=active]:text-[#7c3aed] data-[state=active]:shadow-sm transition-all h-full text-xs sm:text-sm font-medium">
              Quotation
            </TabsTrigger>
            <TabsTrigger value="payments" className="flex-1 px-4 sm:px-0 rounded-xl data-[state=active]:bg-white data-[state=active]:text-[#7c3aed] data-[state=active]:shadow-sm transition-all h-full text-xs sm:text-sm font-medium">
              Payments
            </TabsTrigger>
            <TabsTrigger value="expenses" className="flex-1 px-4 sm:px-0 rounded-xl data-[state=active]:bg-white data-[state=active]:text-[#7c3aed] data-[state=active]:shadow-sm transition-all h-full text-xs sm:text-sm font-medium">
              Expenses
            </TabsTrigger>
            <TabsTrigger value="portfolio" className="flex-1 px-4 sm:px-0 rounded-xl data-[state=active]:bg-white data-[state=active]:text-[#7c3aed] data-[state=active]:shadow-sm transition-all h-full text-xs sm:text-sm font-medium">
              Portfolio
            </TabsTrigger>
          </TabsList>
        </div>
        ...
        {/* 1. QUOTATION TAB */}
        <TabsContent value="quotation" className="space-y-6 outline-none animate-in fade-in slide-in-from-bottom-4 duration-500">
          <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5">
            <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4">
              <div>
                ...
                <CardDescription>Detailed quotation of all services and add-ons.</CardDescription>
              </div>
              <div className="flex flex-wrap sm:flex-nowrap items-center gap-3">
                <div className="flex items-center gap-2 border-slate-200 sm:pr-4 sm:border-r">
                  <Label htmlFor="additional-charges-toggle" className="text-sm font-medium text-slate-600 cursor-pointer">
                    Additional Charges
                  </Label>
                  <Switch 
                ...
                <table className="w-full text-sm text-left block sm:table">
                  ...
                  <tbody className="divide-y divide-slate-100 block sm:table-row-group">
                    {booking.services.map((service, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors block sm:table-row p-4 sm:p-0">
                        ...
                        <td className="sm:px-6 sm:py-4 sm:text-center block sm:table-cell mb-1 sm:mb-0 text-slate-500 sm:text-slate-800">
                          <span className="sm:hidden font-medium mr-2">Qty:</span>
                          {service.quantity}
                        </td>
                        <td className="sm:px-6 sm:py-4 sm:text-right text-slate-500 block sm:table-cell mb-1 sm:mb-0">
                          <span className="sm:hidden font-medium mr-2">Rate:</span>
            ...
            <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5 mt-6 animate-in slide-in-from-top-4 fade-in duration-300">
              <CardHeader className="flex flex-row items-center justify-between pb-4">
                <div>
                  ...
                  <CardDescription>Add custom expenses like travel, early morning fees, etc.</CardDescription>
                </div>
              </CardHeader>
                  ...
                  <table className="w-full text-sm text-left block sm:table">
                    <thead className="bg-slate-50 text-slate-500 hidden sm:table-header-group">
                      <tr>
                        ...
                        <th className="px-4 py-3 font-medium text-right w-32">Rate</th>
                    ...
                    <tbody className="divide-y divide-slate-100 block sm:table-row-group">
                      {booking.additional_charges.map(charge => (
                        <tr key={charge.id} className="hover:bg-slate-50/50 transition-colors block sm:table-row relative p-4 sm:p-0">
                          {editingChargeId === charge.id ? (
                            <>
                              <td className="sm:px-4 sm:py-2 block sm:table-cell">
                                <span className="sm:hidden text-xs font-medium text-slate-500 mb-1 block">Charge Name</span>
                                <Input value={editChargeName} onChange={e => setEditChargeName(e.target.value)} className="h-9 sm:h-8 text-sm" />
                              </td>
                              <td className="sm:px-4 sm:py-2 block sm:table-cell flex-1 mt-2 sm:mt-0">
                                  <span className="sm:hidden text-xs font-medium text-slate-500 mb-1 block">Qty</span>
                                  <Input type="number" value={editChargeQty} onChange={e => setEditChargeQty(e.target.value)} className="h-9 sm:h-8 text-sm text-center" />
                                </td>
                                <td className="sm:px-4 sm:py-2 block sm:table-cell flex-[2] mt-2 sm:mt-0">
                                  <span className="sm:hidden text-xs font-medium text-slate-500 mb-1 block">Rate</span>
                                  <Input type="number" value={editChargeRate} onChange={e => setEditChargeRate(e.target.value)} className="h-9 sm:h-8 text-sm text-right" />
                                </td>
                              <td className="sm:px-4 sm:py-3 text-right font-semibold text-slate-800 block sm:table-cell pt-2 sm:pt-3">
                                <span className="sm:hidden font-medium text-slate-500 mr-2">Total:</span>
                              ...
                              <td className="sm:px-4 sm:py-2 flex gap-2 justify-end sm:justify-center sm:table-cell mt-2 sm:mt-0">
                                ...
                                >
                                  {isUpdatingCharge ? <Loader2 className="size-4 sm:size-3.5 mr-1 sm:mr-0 animate-spin" /> : <Check className="size-4 sm:size-3.5 mr-1 sm:mr-0" />} <span className="sm:hidden">Save</span>
                                </Button>
                                <Button size="sm" variant="outline" className="h-9 sm:size-7 sm:p-0 text-slate-600 border-slate-200 hover:bg-slate-100" onClick={() => setEditingChargeId(null)}>
                                  <X className="size-4 sm:size-3.5 mr-1 sm:mr-0" /> <span className="sm:hidden">Cancel</span>
                                </Button>
                              </td>
                            </>
                          ) : (
                            <>
                              <td className="sm:px-4 sm:py-3 font-medium text-slate-800 block sm:table-cell mb-1 sm:mb-0 pr-16 sm:pr-4">{charge.charge_name}</td>
                              <td className="sm:px-4 sm:py-3 sm:text-center block sm:table-cell text-sm text-slate-500 sm:text-slate-800">
                                <span className="sm:hidden font-medium mr-2">Qty:</span>{charge.quantity}
                              </td>
                              <td className="sm:px-4 sm:py-3 sm:text-right text-slate-500 block sm:table-cell text-sm">
                                <span className="sm:hidden font-medium mr-2">Rate:</span>₹{Number(charge.rate).toLocaleString()}
                              </td>
                              <td className="sm:px-4 sm:py-3 sm:text-right font-semibold text-slate-800 block sm:table-cell border-t sm:border-0 border-slate-100 mt-2 pt-2 sm:mt-0 sm:pt-3">
                                <span className="sm:hidden font-medium text-slate-500 mr-2">Total:</span>₹{(Number(charge.rate) * charge.quantity).toLocaleString()}
                              </td>
                              <td className="absolute top-3 right-3 sm:static sm:px-4 sm:py-3 text-center flex gap-1 justify-center block sm:table-cell">
                                ...
                                >
                                  {deletingChargeId === charge.id ? <Loader2 className="size-4 sm:size-3.5 animate-spin" /> : <Trash className="size-4 sm:size-3.5" />}
                                </Button>
                              </td>
                            </>
                          )}
                      ...
                      {booking.additional_charges.length === 0 && (
                        <tr className="block sm:table-row">
                          <td colSpan={5} className="px-4 py-8 text-center text-slate-500 block sm:table-cell">
            ...
            <CardContent className="p-6">
              <div className="max-w-sm ml-auto space-y-3">
                  ...
                  {isEditingDiscount ? (
                    <div className="flex items-center gap-2">
                      <Input 
                        ...
                        autoFocus
                      />
                      <Button size="icon" variant="ghost" className="size-6 text-emerald-600 hover:bg-emerald-100" onClick={handleUpdateDiscount} disabled={isUpdatingDiscount}>
                        {isUpdatingDiscount ? <Loader2 className="size-3.5 animate-spin" /> : <Check className="size-3.5" />}
                      </Button>
                      <Button size="icon" variant="ghost" className="size-6 text-slate-400 hover:bg-slate-100" onClick={() => setIsEditingDiscount(false)}>
                        <X className="size-3.5" />
                      </Button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 cursor-pointer group" onClick={() => { setTempDiscount(String(booking.discount)); setIsEditingDiscount(true); }}>
                      <span className="font-medium text-rose-600">-₹{Number(booking.discount).toLocaleString()}</span>
                ...
                <div className="flex justify-between items-center pt-1">
                  ...
                  <span className="text-2xl font-bold text-[#7c3aed]">₹{grandTotal.toLocaleString()}</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {/* 2. PAYMENTS TAB */}
        <TabsContent value="payments" className="space-y-6 outline-none animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid gap-6 md:grid-cols-[1fr_350px]">
            <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5 h-fit">
              <CardHeader className="flex flex-row items-center justify-between pb-4">
                <div>
                  <CardTitle className="text-lg">Payment History</CardTitle>
                  <CardDescription>Track all transactions for this booking.</CardDescription>
                </div>
                <Button 
                    ...
                    <div key={payment.id} className="flex gap-4 group">
                      ...
                      <div className="flex-1 pb-6">
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                          <div>
                            <h4 className="font-semibold text-slate-800">{payment.payment_type} Payment</h4>
                            <p className="text-xs text-slate-500 mt-0.5">via {payment.payment_method} • {new Date(payment.payment_date).toLocaleDateString()}</p>
                            {payment.remark && (
                              <p className="text-xs text-slate-400 mt-1 italic">{payment.remark}</p>
                            )}
                            <Badge variant="outline" className="mt-2 text-[10px] text-emerald-600 border-emerald-200 bg-emerald-50 uppercase tracking-wider">
                              Completed
                            </Badge>
                          </div>
                          <div className="flex items-center gap-1 self-start sm:self-auto bg-slate-50 sm:bg-transparent p-1.5 sm:p-0 rounded-xl border border-slate-100 sm:border-transparent mt-2 sm:mt-0">
                            <span className="font-bold text-slate-900 mr-2 ml-1">₹{Number(payment.amount).toLocaleString()}</span>
                            <Button
                              size="icon" variant="ghost"
                              className="size-8 sm:size-7 rounded-lg text-slate-400 hover:text-[#7c3aed] hover:bg-purple-50"
                              onClick={() => {
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  const res = await fetch(`/api/bookings/${bookingId}/payments?paymentId=${payment.id}`, { method: "DELETE" })
            ...
            <div className="space-y-6">
              <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5 bg-gradient-to-br from-[#7c3aed] to-[#5b21b6] text-white">
                <CardHeader className="pb-2">
                  <CardTitle className="text-lg font-medium text-white/90">Amount Due</CardTitle>
                </CardHeader>
                <CardContent>
                  <h2 className="text-4xl font-bold tracking-tight mb-4">₹{dueAmount.toLocaleString()}</h2>
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs text-purple-200">
                      <span>Progress</span>
                      <span>{Math.round(paidPercentage)}% Paid</span>
                    </div>
                    <Progress value={paidPercentage} className="h-2 bg-purple-900/50" />
                  </div>
              ...
              <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5">
                <CardContent className="p-5 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-slate-800">Total Billed</p>
        ...
        {/* 3. EXPENSES TAB */}
        <TabsContent value="expenses" className="space-y-6 outline-none animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid gap-6 xl:grid-cols-[1fr_320px_320px] lg:grid-cols-3">
            ...
            <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5 h-fit">
              <CardHeader className="flex flex-row items-center justify-between pb-4">
                <div>
                  ...
                  <CardDescription>Costs incurred for this specific booking.</CardDescription>
                </div>
                <Button 
              ...
              <CardContent>
                <div className="space-y-4">
                  {expenses.map((expense) => (
                    <div key={expense.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border border-slate-100 hover:bg-slate-50 transition-colors gap-3 sm:gap-4">
                      <div className="flex items-center gap-4">
                        <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-rose-50 text-rose-500">
                          <TrendingDown className="size-5" />
                        </div>
                        <div>
                          <h4 className="font-semibold text-slate-800">{expense.expense_name}</h4>
                          <div className="flex flex-wrap items-center gap-2 mt-0.5">
                            <span className="text-xs text-slate-400 shrink-0">{new Date(expense.expense_date).toLocaleDateString()}</span>
                            {expense.description && (
                              <span className="text-xs text-slate-400 truncate max-w-[200px] sm:max-w-[160px]">• {expense.description}</span>
                            )}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 self-start sm:self-auto bg-slate-50 sm:bg-transparent p-1.5 sm:p-0 rounded-xl border border-slate-100 sm:border-transparent ml-14 sm:ml-0">
                        <span className="font-bold text-slate-900 mr-2 ml-1">₹{Number(expense.amount).toLocaleString()}</span>
                        <Button
                          size="icon" variant="ghost"
                          className="size-8 sm:size-7 rounded-lg text-slate-400 hover:text-[#7c3aed] hover:bg-purple-50"
                          onClick={() => {
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const res = await fetch(`/api/bookings/${bookingId}/expenses?expenseId=${expense.id}`, { method: "DELETE" })
            ...
            <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5 h-fit">
              <CardHeader className="pb-3">
                <CardTitle className="text-lg font-medium">Custom Expenses</CardTitle>
                <CardDescription>Track additional expense categories.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex gap-2">
                  <Input 
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleAddCustomExpense()}
                  />
                  <Button 
                    size="sm" 
                  ...
                  >
                    {isAddingCustomExpense ? <Loader2 className="size-4 animate-spin" /> : "Add"}
                  </Button>
                </div>
                
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                  {customExpenses.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-2">No custom expenses added.</p>
                  ) : (
                    customExpenses.map(expense => (
                      <div key={expense.id} className="flex items-center justify-between p-2.5 rounded-xl border border-slate-100 bg-white hover:border-purple-100 transition-colors gap-2">
                        {editingCustomExpenseId === expense.id ? (
                          <div className="flex w-full items-center gap-2">
                            <Input 
                              value={editingCustomExpenseName}
                              onChange={(e) => setEditingCustomExpenseName(e.target.value)}
                              className="h-8 text-sm rounded-lg border-purple-200"
                              autoFocus
                              onKeyDown={(e) => e.key === 'Enter' && handleUpdateCustomExpense()}
                            />
                            <Button size="icon" variant="ghost" className="size-8 rounded-lg text-emerald-600 hover:bg-emerald-50 shrink-0" onClick={handleUpdateCustomExpense} disabled={isUpdatingCustomExpense}>
                              {isUpdatingCustomExpense ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />}
                            </Button>
                            <Button size="icon" variant="ghost" className="size-8 rounded-lg text-slate-400 hover:bg-slate-50 shrink-0" onClick={() => setEditingCustomExpenseId(null)}>
                              <X className="size-4" />
                            </Button>
                          </div>
                        ) : (
                          <>
                            <span className="text-sm font-medium text-slate-700 break-words flex-1 line-clamp-2">{expense.name}</span>
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <Button 
                                ...
                                disabled={deletingCustomExpenseId === expense.id}
                              >
                                {deletingCustomExpenseId === expense.id ? <Loader2 className="size-4 sm:size-3.5 animate-spin" /> : <Trash className="size-4 sm:size-3.5" />}
                              </Button>
                            </div>
                          </>
                        )}
            ...
            <Card className="rounded-[1.75rem] border-slate-100 shadow-md shadow-purple-950/5 h-fit">
              ...
              <CardContent className="space-y-5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-slate-800">₹{grandTotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-emerald-600">₹{netProfit.toLocaleString()}</span>
                </div>
                
                <div className="pt-4 mt-4 border-t border-slate-100">
                  <p className="text-xs text-slate-500 mb-2">Profit Margin</p>
                  <div className="flex items-center gap-3">
                    <Progress value={Number(profitMargin)} className="h-2.5 bg-slate-100 text-emerald-500" />
                    <span className="text-sm font-bold text-slate-700">{profitMargin}%</span>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        {/* 4. PORTFOLIO TAB */}
        <TabsContent value="portfolio" className="space-y-6 outline-none animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <AppModal
        ...
        footer={!updateSent ? (
          ...
          <Button variant="outline" className="h-10 rounded-xl" onClick={() => setUpdateOpen(false)}>
            Close
          </Button>
        )}
      >
        {!updateSent ? (
          <div className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                <div className={`p-2 rounded-xl ${updateType === "quotation" ? "bg-[#7c3aed] text-white" : "bg-slate-100 text-slate-500"}`}>
                  <FileText className="size-4" />
                </div>
                <div>
                  <h4 className="font-semibold text-sm text-slate-800">Quotation</h4>
                  <p className="text-xs text-slate-500 mt-0.5">Send a PDF link of the detailed quote.</p>
                </div>
              </div>
              ...
              >
                <div className={`p-2 rounded-xl ${updateType === "reminder" ? "bg-[#7c3aed] text-white" : "bg-slate-100 text-slate-500"}`}>
                  <Wallet className="size-4" />
                </div>
                <div>
                  <h4 className="font-semibold text-sm text-slate-800">Payment Reminder</h4>
                  <p className="text-xs text-slate-500 mt-0.5">Remind client of pending ₹{dueAmount.toLocaleString()}.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        footer={!paymentSuccess ? (
          <>
            ...
            <Button 
              ...
              disabled={!paymentAmount || Number(paymentAmount) <= 0 || Number(paymentAmount) > dueAmount || isSavingPayment}
              onClick={async () => {
            ...
            >
              {isSavingPayment ? <Loader2 className="mr-2 size-4 animate-spin" /> : null}
              Save Payment
            </Button>
          </>
        ) : null}
      >
        {!paymentSuccess ? (
          ...
          <div className="py-12 flex flex-col items-center justify-center text-center animate-in zoom-in duration-300">
            ...
            <h3 className="text-xl font-bold text-slate-800">Payment Logged!</h3>
            <p className="text-slate-500 mt-2 max-w-sm">
              ₹{Number(paymentAmount).toLocaleString()} was successfully added to this booking.
            </p>
          </div>
        )}
      </AppModal>
        ...
        footer={!expenseSuccess ? (
          <>
            ...
            <Button 
              ...
              disabled={!expenseName || !expenseAmount || Number(expenseAmount) <= 0 || isSavingExpense}
              onClick={async () => {
                setIsSavingExpense(true)
                try {
                  const res = await fetch(`/api/bookings/${bookingId}/expenses`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
            ...
            >
              {isSavingExpense ? <Loader2 className="mr-2 size-4 animate-spin" /> : null}
              Log Expense
            </Button>
          </>
        ) : null}
      >
        {!expenseSuccess ? (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <FloatingDropdown
                  ...
                  value={expenseName}
                >
                  <DropdownMenuRadioGroup value={expenseName} onValueChange={setExpenseName}>
                    {customExpenses.length === 0 ? (
                      <div className="px-2 py-1.5 text-sm text-slate-500">No categories added</div>
                    ) : (
                      customExpenses.map(cat => (
                        <DropdownMenuRadioItem key={cat.id} value={cat.name} className="rounded-xl">
                          {cat.name}
                        </DropdownMenuRadioItem>
                      ))
                    )}
                ...
                {customExpenses.length === 0 && (
                  <p className="text-[10px] text-slate-400 mt-1">Add categories in the Custom Expenses card first.</p>
                )}
              </div>
            ...
            <DatePicker value={expenseDate} onChange={setExpenseDate} className="bg-white" />
          ...
          <div className="py-12 flex flex-col items-center justify-center text-center animate-in zoom-in duration-300">
            ...
            <h3 className="text-xl font-bold text-slate-800">Expense Logged!</h3>
            <p className="text-slate-500 mt-2 max-w-sm">
              ₹{Number(expenseAmount).toLocaleString()} was successfully added to this booking's expenses.
            </p>
          </div>
        )}
      </AppModal>
      ...
      <AppModal
        ...
        title="Edit Payment"
        description={editingPayment ? `Editing ${editingPayment.payment_type} payment of ₹${Number(editingPayment.amount).toLocaleString()}` : ""}
        footer={
          <>
            <Button variant="outline" className="h-11 rounded-2xl border-slate-200 bg-white hover:bg-slate-50 text-slate-700" onClick={() => { setEditPaymentOpen(false); setEditingPayment(null) }}>
              Cancel
            </Button>
            <Button
              className="h-11 rounded-2xl bg-emerald-500 hover:bg-emerald-600 shadow-md shadow-emerald-500/20 text-white"
              disabled={!editPaymentAmount || Number(editPaymentAmount) <= 0 || isSavingPayment}
              onClick={async () => {
      ...
      <AppModal
        ...
        title="Edit Expense"
        description={editingExpense ? `Editing "${editingExpense.expense_name}"` : ""}
        footer={
          <>
            <Button variant="outline" className="h-11 rounded-2xl border-slate-200 bg-white hover:bg-slate-50 text-slate-700" onClick={() => { setEditExpenseOpen(false); setEditingExpense(null) }}>
              Cancel
            </Button>
            <Button
              className="h-11 rounded-2xl bg-rose-500 hover:bg-rose-600 shadow-md shadow-rose-500/20 text-white"
              disabled={!editExpenseName.trim() || !editExpenseAmount || Number(editExpenseAmount) <= 0 || isSavingExpense}
              onClick={async () => {
      ...
      <AppModal
        ...
        footer={
          <>
            <Button variant="outline" className="h-11 rounded-2xl border-slate-200 bg-white hover:bg-slate-50 text-slate-700" onClick={() => setEditServicesOpen(false)}>
              Cancel
            </Button>
            <Button
            ...
            >
              {isSavingServices ? <Loader2 className="mr-2 size-4 animate-spin" /> : null}
              Save Services
            </Button>
          </>
        }
        ...
        <div className="space-y-6">
            ...
            <table className="w-full text-sm text-left block sm:table">
              ...
              <tbody className="divide-y divide-slate-100 block sm:table-row-group">
                {editingServices.map((service, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors block sm:table-row relative p-4 sm:p-0">
                    ...
                    <td className="sm:px-4 sm:py-2 block sm:table-cell mt-2 sm:mt-0">
                      <span className="sm:hidden text-xs font-medium text-slate-500 mb-1 block">Unit Price (₹)</span>
                      <Input 
                        type="number"
                        min="0"
                        value={service.price}
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div className="w-full sm:w-2/3 space-y-2">
              <Label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Add a Service</Label>
              <DropdownMenu>
                <DropdownMenuTrigger className="flex h-11 w-full items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 text-sm hover:bg-slate-50 transition-colors">
                  <span className="text-slate-500">Select a service to add...</span>
                  <ChevronDown className="size-4 text-slate-400" />
                </DropdownMenuTrigger>
                <DropdownMenuContent className="w-full max-w-[300px] rounded-2xl max-h-60 overflow-y-auto">
                  {availableServices.filter(s => !editingServices.some(es => es.id === s.id)).map(s => (
                    <DropdownMenuItem 
                      key={s.id} 
                      className="rounded-xl cursor-pointer"
                      onClick={() => {
                        setEditingServices([...editingServices, { ...s, quantity: 1 }])
                      }}
                    >
                      <div className="flex items-center justify-between w-full">
                        <span>{s.service_name}</span>
                        <span className="text-slate-500 text-xs">₹{s.price.toLocaleString()}</span>
                      </div>
                    </DropdownMenuItem>
                  ))}
                  {availableServices.filter(s => !editingServices.some(es => es.id === s.id)).length === 0 && (
                    <div className="p-3 text-sm text-slate-500 text-center">No more services available</div>
                  )}
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
            ...
            <div className="text-right">
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-1">New Total</p>
              <p className="text-2xl font-bold text-[#7c3aed]">
                ₹{editingServices.reduce((acc, s) => acc + (s.price * s.quantity), 0).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
